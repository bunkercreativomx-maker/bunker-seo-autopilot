...
export function StrategyDashboard({ websiteId, data, canEdit }: { websiteId: string; data: StrategyData; canEdit: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section id="opportunities">
        <Card>
          <CardHeader title="Opportunities" subtitle="Content gaps and recommended actions. This phase does not generate articles." />
          <CardBody className="space-y-3">
            ...
            {data.opportunities.map((item) => (
              <OpportunityCard key={item.id} item={item} websiteId={websiteId} clusters={clusterOptions} priorities={priorityOptions} canEdit={canEdit} />
            ))}
          </CardBody>
        </Card>
      </section>
        ...
        <Card>
          ...
          <CardBody className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            {(["30", "60", "90"] as const).map((horizon) => (
              <PlanColumn key={horizon} horizon={horizon} items={data.plan.filter((item) => planHorizon(item) === horizon)} websiteId={websiteId} canEdit={canEdit} />
            ))}
          </CardBody>
        </Card>
      </section>
...
function OpportunityCard({ item, websiteId, clusters, priorities, canEdit }: { item: ContentOpportunity; websiteId: string; clusters: Array<{ value: string; label: string }>; priorities: Array<{ value: string; label: string }>; canEdit: boolean }) {
  const keyword = item.expand?.keyword;
  return <article className="rounded-lg border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-semibold text-slate-900">{display(item.title_suggestion || item.recommended_url)}</h4><p className="text-xs text-slate-500">Target: {display(keyword?.keyword)} · Volume: {metric(keyword?.search_volume)} · Difficulty: {metric(keyword?.keyword_difficulty)}</p></div><DecisionButtons websiteId={websiteId} recordId={item.id} collection="opportunities" status={item.status} canEdit={canEdit} /></div>{item.reason && <p className="mt-2 text-sm text-slate-600">{item.reason}</p>}<div className="mt-3 grid gap-2 md:grid-cols-4"><div className="rounded border border-slate-200 px-2 py-1 text-xs">Intent: {display(keyword?.intent)}</div><ChangeSelect websiteId={websiteId} recordId={item.id} collection="opportunities" operation="change_cluster" value={item.cluster} values={clusters} canEdit={canEdit} /><ChangeSelect websiteId={websiteId} recordId={item.id} collection="opportunities" operation="change_priority" value={item.priority} values={priorities} canEdit={canEdit} /><ChangeSelect websiteId={websiteId} recordId={item.id} collection="opportunities" operation="change_action" value={item.opportunity_type} values={OPPORTUNITY_ACTIONS.map((value) => ({ value, label: value.replaceAll("_", " ") }))} canEdit={canEdit} /></div></article>;
}
